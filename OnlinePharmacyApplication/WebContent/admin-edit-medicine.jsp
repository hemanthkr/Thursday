...
<body>
	...
	medicineModel.setMedType(medType);
	medicineModel.setUnitValue(unitValue);
	medicineModel.setMedPrice(medPrice);
	medicineModel.setUnit(unit);
	medicineModel.setMedExpiryDate(medExpiryDate);
	medicineModel.setInStock(inStock);
	...
	try{
		...
		if(flag==0){ */
		String insQuery = "update medicine_table set medName=?, medType=?, unitValue=?, medPrice=?, unit=?, medExpiryDate=?, inStock=? where medName='"+medicineModel.getMedName()+"'";
